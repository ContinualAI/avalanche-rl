...
def make_logger(log_every: int, train_window_size: int = 10,
                eval_window_size: int = 4,
                tracked_metrics: List[str] = ['reward', 'ep_length'],
                tracked_stats: List[str] = ['mean', 'max', 'std']) \
                    -> RLEvaluationPlugin:
    """
    Creates a metric logger which can be used as `evaluator` to track,
    record and print out metrics.

    Args:
        log_every (int): How often should we print out metrics,
                in iteration steps.
        train_window_size (int, optional): Size of the `moving_window_stat`
                metric tracker during training. Defaults to 10.
        eval_window_size (int, optional): Size of the `moving_window_stat`
                metric tracker during evaluation. Defaults to 4.
        tracked_metrics (List[str], optional): Metrics to track.
                Defaults to ['reward', 'ep_length'].
        tracked_stats (List[str], optional): Aggregated stats to compute on the
                moving window. Defaults to ['mean', 'max', 'std'].
    ...
    return RLEvaluationPlugin(*metrics,
                              loggers=[TqdmWriteInteractiveLogger(
                                        log_every=log_every)])
